...
def s21skewed(freq, F0, Qr, Qcre, Qcim, A):
    """
...
def s21skewed_nonlinear(freq, F0, Qr, Qcre, Qcim, A, a):
    """
    ...
    roots = np.array(cardan( * [4.0, -4.0*y0, 1.0, -(y0 + a)] )).T

    # Extract real roots only
    y = roots[np.where(abs(roots.imag) < 1e-15)].real
    return abs(A*(1 - Qr/(Qcre + 1j*Qcim)/(1 + 2j*y)))

def fitmags21(freq, s21, approxQr = 1.e5,  nonlinear=False, normalise=False, db=False, make_plot=False):
    ...
    if nonlinear==True:
        init = [freq[np.argmin(abs(s21))], approxQr/2, approxQr, 100., abs(s21).mean(), 0.1]
        param_bounds = (0, [np.inf]*5 + [np.around(4*np.sqrt(3)/9, 3)] ) # set the parameter bounds

        s21fitp, s21cov = curve_fit(s21skewed_nonlinear, freq, abs(s21), p0=init, bounds = param_bounds)

        errs = np.sqrt(diag(s21cov))
        fits21 = s21skewed_nonlinear(freq, *s21fitp)
        a = s21fitp[-1]
    ...
    if make_plot:
        fig21, axs21 = subplots(1,1, num=1)
        if normalise: fits21 = fits21/s21fitp[-1]; s21 = abs(s21/s21fitp[-1])
        elif db: fits21 = 20*log10(fits21); s21 = 20*log10(abs(s21))
    ...
    return array([f0, Qr, Qc, Qi, a]), array([errf0, errQr, errQc, errQi, erra]), fits21
#
